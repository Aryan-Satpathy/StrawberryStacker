...
class control:
    ...
    def __init__(self, name = '/edrone0'):
        # Initialise rosnode
...
def main(name : str) -> None:
    ...
    rospy.Subscriber(name + '/mavros/local_position/pose', PoseStamped, stateMt.posiCallBack)
    rospy.Subscriber(name + '/gripper_check', std_msgs.msg.String, stateMt.gripperCallBack)
    Subscriber(name + '/camera/image_raw', Image, stateMt.camCallBack)

    # Initialize publishers
    local_pos_pub = rospy.Publisher(name + '/mavros/setpoint_position/local', PoseStamped, queue_size=10)
    local_vel_pub = rospy.Publisher(name + '/mavros/setpoint_velocity/cmd_vel', Twist, queue_size=10)
    ...
    rospy.Subscriber(name + "/mavros/state",State, stateMt.stateCb)

    # Service proxy
    # Calling to /mavros/param/get to get the param value and check if failsafe has been disabled successfully
    paramState = rospy.ServiceProxy(name + '/mavros/param/get', ParamGet) # Creating a proxy service for the rosservice named /mavros/cmd/arming for arming the drone
