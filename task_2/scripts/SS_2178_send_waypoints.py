...
# Calling the rosservices
    ...
    def auto_set_mode(self):
        ...
        msg = SetModeRequest()
        msg.base_mode = 0
        msg.custom_mode = 'AUTO.MISSION'
        try:
            modeService = rospy.ServiceProxy('mavros/set_mode', mavros_msgs.srv.SetMode) # Creating a proxy service for the rosservice named /mavros/set_mode for arming the drone 
            modeService.call(msg)
        except rospy.ServiceException as e:
    ...
    def wpPush(self,index,wps):
        ...
        try:
            wpPushService = rospy.ServiceProxy('mavros/mission/push', mavros_msgs.srv.WaypointPush) # Creating a proxy service for the rosservice named /mavros/mission/push for arming the drone 
            wpPushService(index, wps)
        except rospy.ServiceException as e:
...
def main():
    ...
    # Take off at home location
    w = wayp0.setWaypoints(3,22,True,True,0.0,0.0,0.0,float('nan'),19.134423, 72.911763, 10)
    wps += [w] * 1
    
    # Go to Waypoint 1 as mentioned in PS
    w = wayp1.setWaypoints(3,16,False,True,0.0,0.0,0.0,float('nan'),19.134641,72.911706,10)
    wps += [w] * 1
    
    # Go to Waypoint 2 as mentioned in PS
    w = wayp2.setWaypoints(3,16,False,True,0.0,0.0,0.0,float('nan'),19.134617, 72.911886, 10)
    wps += [w] * 1
    
    # Go to Waypoint 3 as mentioned in PS
    w = wayp3.setWaypoints(3,16,False,True,0.0,0.0,0.0,float('nan'),19.134434, 72.911817, 10)
    wps += [w] * 1

    # Go to Waypoint 4 i.e, home location + 10 meters altitude
    w = wayp4.setWaypoints(3,16,False,True,0.0,0.0,0.0,float('nan'),19.134423, 72.911763, 10)
    wps += [w] * 1

    #  Land there
    w = wayp5.setWaypoints(3,21,False,False,0.0,0.0,0.0,float('nan'),19.134423, 72.911763, 0)
    wps += [w] * 1
    ...
    while not stateMt.state.mode=="AUTO.MISSION":
        md.auto_set_mode()
        rate.sleep()
        print ("Trying to set AUTO.MISSION mode")

    print('Successfully set to auto mission mode')

    print('No output expected after this')
