...
class control:
    ...
    def land_set_mode(self) -> None:
        ...
        try:
            ...
            print ("Service set mode call failed: %s"%e)
    def gripper(self, arg : bool) -> None :
...
class stateMoniter:
    ...
    def gripperCallBack(self, msg : bool) -> None :
        self.canPick = msg
...
def coverSetpoints(setpointsIndices : list, setpoints : list, stateMt : stateMoniter, local_pos_pub : Publisher, local_vel_pub : Publisher, rate : rospy.Rate, ctl : control, paramState : ServiceProxy) -> None :
...
def enterOffboard(dummySetpoint : tuple, stateMt : stateMoniter, ctl : control, paramState : rospy.ServiceProxy, local_pos_pub : Publisher, local_vel_pub : Publisher, rate : rospy.Rate) -> None :
    ...
    while not rospy.is_shutdown() : 
        ...
        if not stateMt.state.armed : 
            ctl.setArm()
        if (paramState('COM_RCL_EXCEPT').value.integer >> 2) & 1 : 
            ctl.setFailSafe()
...
def main() -> None:
    ...
    addSetpoint((0, 0, 3), (0, 0, 3), setpoints)                # Takeoff setpoint                             -
    addSetpoint((3, 0, 3), (3, 0, 0), setpoints)                # 1st setpoint                                  |   Before picking up the object
    ...
    coverSetpoints(setpointsBeforePick, setpoints, stateMt, local_pos_pub, local_vel_pub, rate, ctl, paramState)

    while True :
        land(ctl, stateMt, rate)
        print('Landed')
        cond = False    
        start = rospy.timer.time.time()

        while (rospy.timer.time.time() - start) < TIMEOUTTHRESH : 
            if stateMt.canPick and abs(stateMt.x - setpoints[setpointsBeforePick[-1]][0].pose.position.x) < linPrecision and abs(stateMt.y - setpoints[setpointsBeforePick[-1]][0].pose.position.y) < linPrecision :
                break
        ...
        if cond :
            print("Timeout, retrying Landing on the box.")
            tempSetpoints = setpointsBeforePick[-2 : ]
            coverSetpoints(tempSetpoints, setpoints, stateMt, local_pos_pub, local_vel_pub, rate, ctl, paramState)
        else :
            break

    print('Picking up package now.')

    # Pick up code here
    lastSetpoint = setpoints[setpointsBeforePick[-1]]
    while not rospy.is_shutdown() :

        if stateMt.canPick :
            start = rospy.timer.time.time()
            
            while (rospy.timer.time.time() - start) < 1 : #  Keep calling the service for 1 second, just to make sure. (I had cases where /gripper_check yields true but it doesn't actually pick the package)
                ctl.gripper(True)
                rate.sleep()
    ...
    enterOffboard(lastSetpoint, stateMt, ctl, paramState, local_pos_pub, local_vel_pub, rate) 

    coverSetpoints(setpointsAfterPick, setpoints, stateMt, local_pos_pub, local_vel_pub, rate, ctl, paramState)
    ...
    while True :
        ...
        while (rospy.timer.time.time() - start) < TIMEOUTTHRESH : 
            if stateMt.canPick and abs(stateMt.x - setpoints[setpointsAfterPick[-1]][0].pose.position.x) < linPrecision and abs(stateMt.y - setpoints[setpointsAfterPick[-1]][0].pose.position.y) < linPrecision :
                break
        ...
        if cond :
            print("Timeout, retrying Landing on the target location.")
            tempSetpoints = setpointsBeforePick[-2 : ]
            coverSetpoints(tempSetpoints, setpoints, stateMt, local_pos_pub, local_vel_pub, rate, ctl, paramState)
        else :
            break
    
    print('Droping package now.')

    # Drop code here
    lastSetpoint = setpoints[setpointsAfterPick[-1]]
    ctl.gripper(False)
    ...
    coverSetpoints(setpointsAfterDropping, setpoints, stateMt, local_pos_pub, local_vel_pub, rate, ctl, paramState)
